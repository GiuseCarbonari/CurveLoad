...
export function RefreshControl({
  ...
  /** Esito del sync manuale (true = ok) per concatenare eventuale lavoro extra del chiamante. */
  ...
  async function handleRefresh() {
    ...
    try {
      ...
      setStatus("fresh");
      // Se il chiamante gestisce lavoro extra post-sync (es. rigenera piano),
      // il router.refresh() finale spetta a lui; altrimenti lo facciamo qui.
      if (onSyncDone) {
        onSyncDone(true);
      } else {
        router.refresh();
  ...
  return (
    <div id="tour-refresh">
      <div
        aria-live="polite"
        className="flex items-center justify-end gap-1.5 text-[11.5px]"
      >
