...
export function RunnerCard({ runner }: RunnerCardProps) {
  ...
  return (
    <div className="rounded-metric border border-border bg-surface px-5 py-5">
      ...
      {/* Zone di passo e predizioni si sono spostate in /terrain, accanto
          alle previsioni Riegel da gare reali — lì si pensa già alla gara
          target, qui resta solo lo scatto di fitness. */}
      <Link
        href="/terrain"
        className="mt-4 flex items-center justify-between rounded-metric border border-border bg-surface-2 px-4 py-3 text-sm text-secondary transition-colors hover:bg-surface-2/70 hover:text-foreground"
      >
        Vedi il tuo pacing e le previsioni di gara
