/**
 * Run Workout Library — catalogo template per la CORSA (dato PURO, nessuna
 * logica, come workout-library.ts). Recuperato dal commit precedente la sua
 * rimozione (bbcd7e4^) e riallineato al tipo condiviso `WorkoutTemplate`:
 * gli id qui presenti sono l'UNICA fonte dei `library_id` corsa, letti da
 * `session-selector.ts` e uniti alla mappa di `getTemplate()` in
 * workout-library.ts. I domain condivisi (endurance, tempo, sweet_spot,
 * threshold, vo2max, anaerobic, race_specific) hanno la stessa semantica
 * fisiologica della bici; le zone in `power_target_zone` sono SOLO
 * l'etichetta (es. "Z2"): il passo effettivo si deriva da CS a runtime
 * (`buildWeek` → `runPaceTarget`), mai hardcoded qui.
 *
...
import type { WorkoutTemplate } from "@/lib/planner/workout-library";

export const RUN_WORKOUT_TEMPLATES: WorkoutTemplate[] = [
...
/** Insieme dei library_id corsa validi (unito a quelli bici in workout-library.ts). */
export const RUN_LIBRARY_IDS: ReadonlySet<string> = new Set(
